...
class AddEnrolledCourse extends StatefulWidget {
  @override
  _AddEnrolledCourseState createState() => _AddEnrolledCourseState();
}

class _AddEnrolledCourseState extends State<AddEnrolledCourse> {
  List<String> courses = [];
          ...
          child: FlatButton(
              onPressed: () async {
                ...
                Section selectedSection = context
                    ...
                    .toList()[0];
                await selectedSection.courseSection.getTutors(context.read<AppSession>().database);
                await context.read<AppSession>().database.rawInsert(
                    "INSERT INTO EnrolledCourse(enrolled_by_user_id,enrolled_section_id) VALUES ( '${context.read<AppSession>().user.idUser}', '${selectedSection.sectionId}');");

                context.read<AppSession>().user.enrolledSections.add(selectedSection);
              ...
              child: Material(
                ...
                child: Container(
                  ...
                  child: Center(
                    child: Text("Add course", style: TextStyle(color: Colors.white, fontSize: 16.0, fontWeight: FontWeight.bold)),
                  ),
                ),
              )),
        )
